...
async def create_chat(request: Request, body: ChatCreate, user=Depends(get_current_user)):
    ...
    doc = {"user_id": ObjectId(user["id"]), "title": body.title or "New chat", "archived": False,
           "created_at": now, "updated_at": now}
    res = await db.chats.insert_one(doc)
    doc["_id"] = res.inserted_id
    return ChatOut(id=str(doc["_id"]), title=doc["title"], archived=False,
                   created_at=now.isoformat(), updated_at=now.isoformat())

@router.patch("/{chat_id}", response_model=ChatOut)
async def patch_chat(chat_id: str, request: Request, body: ChatPatch, user=Depends(get_current_user)):
    db = get_db_dep(request)
