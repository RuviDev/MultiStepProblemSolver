...
try:
    from agent8_memory.hooks import mirror_save_inferred
    def _wrap_func(fn):
        def _w(*args, **kwargs):
            res = fn(*args, **kwargs)
            ts = None; inferred = None
            for a in args:
                if isinstance(a, dict) and 'problem_records' in a:
                    ts = a
            if isinstance(res, dict):
                inferred = res.get('inferred') or res.get('inferred_map') or res.get('auto_insights')
            try:
                mirror_save_inferred(ts, inferred or {})
            except Exception:
                pass
            return res
        return _w
    for name in list(globals().keys()):
        if name.startswith('uia_stage1') and callable(globals()[name]):
            globals()[name] = _wrap_func(globals()[name])
except Exception:
    pass
