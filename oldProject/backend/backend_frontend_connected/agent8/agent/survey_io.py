...
# === Memory mirror auto-wrapper (non-intrusive) ===
try:
    from agent8_memory.hooks import mirror_log_asked, mirror_log_answered
    _export_ref = export_current_survey if 'export_current_survey' in globals() else None
    if _export_ref:
        def export_current_survey(*args, **kwargs):  # type: ignore
            res = _export_ref(*args, **kwargs)
            ts = None
            for a in args:
                if isinstance(a, dict) and 'problem_records' in a:
                    ts = a; break
            try:
                mirror_log_asked(ts, res)
            except Exception:
                pass
            return res
    _apply_ref = apply_survey_answers if 'apply_survey_answers' in globals() else None
    if _apply_ref:
        def apply_survey_answers(*args, **kwargs):  # type: ignore
            res = _apply_ref(*args, **kwargs)
            ts = None; answers = None
            for a in args:
                if isinstance(a, dict) and 'problem_records' in a:
                    ts = a
                if isinstance(a, dict) and any(isinstance(k, str) and '.' in k for k in a.keys()):
                    answers = a
            if answers is None:
                answers = kwargs.get('answers') or kwargs.get('payload') or {}
            try:
                mirror_log_answered(ts, answers)
            except Exception:
                pass
            return res
except Exception:
    pass
# === End memory mirror wrapper ===
